#!/usr/bin/env python3
"""
Scan for and decode ONLY the Victron Energy BLE "Instant Readout" devices
whose MAC addresses are listed in a keys.json file, using a BleuIO Pro USB
BLE dongle. This is the "known devices" variant of victron_bleuio_scanner.py:
instead of surveying every nearby BLE device, it only ever target-scans and
decodes the specific devices you already have keys for.
...
This script:
  ...
  2. Loads keys.json (or --keys-file) and requires it to be non-empty —
     this script has nothing to do without a set of known MAC addresses.
  3. Puts the dongle in dual/central role and runs `AT+GAPSCAN` once, only
     to find out which of the *known* MAC addresses are currently in range
     and what BLE address type (public/random) each one uses. Any other
     device seen in that scan is ignored and never touched again.
  4. For each known MAC address that was seen, runs `AT+SCANTARGET` to grab
     its raw advertisement payload, extracts the Victron manufacturer-data
     payload (company ID 0x02E1, Instant Readout marker 0x10), decrypts it
     with the matching key (AES-128-CTR), and decodes it into real values
     for the device types this script knows how to parse: Battery Monitors
     (BMV / SmartShunt), Solar Chargers (BlueSolar / SmartSolar MPPT), and
     Inverters (Phoenix / Phoenix Smart). Other Victron BLE device types are
     reported with their model name but not decoded.
  5. Known MAC addresses that weren't seen in the scan are reported as
     "not seen" so you know they're out of range or powered off.
...
Put the devices you want to track in keys.json, keyed by MAC address
(case-insensitive):
...
Then run:
    ...
    python3 victron_bleuio_known_devices.py
...
Usage
-----
    python3 victron_bleuio_known_devices.py [--port /dev/tty.usbmodemXXXX]
                                             [--baud 115200]
                                             [--scan-time 10]
                                             [--target-scan-time 4]
                                             [--keys-file keys.json]
...
def run_once(dongle: BleuIODongle, keys: Dict[str, str], args) -> int:
    print(f"Scanning for known devices for {args.scan_time}s...")
    seen = dongle.scan(args.scan_time)

    # Only ever look at MAC addresses we have a key for — everything else
    # seen in the general scan is discarded here.
    known_seen = {mac: info for mac, info in seen.items() if mac in keys}
    missing = sorted(set(keys) - set(known_seen))

    print(
        f"{len(known_seen)}/{len(keys)} known device(s) currently in range"
        + (f", {len(missing)} not seen" if missing else "")
        + "\n"
    )

    found = 0
    for mac, info in sorted(known_seen.items()):
        raw_hex = dongle.read_raw_adv(mac, info["addr_type"], args.target_scan_time)
        name = info["name"] or "(no advertised name)"

        if not raw_hex:
            print(f"{mac}  {name}  -- no advertisement captured this cycle, skipping")
            print()
            continue

        payload = extract_victron_payload(raw_hex)
        if not payload:
            print(f"{mac}  {name}  -- advertisement seen, but no Victron Instant")
            print("  Readout payload found in it (wrong device or out of range?).")
            print()
            continue

        found += 1
        model_id = struct.unpack("<H", payload[2:4])[0]
        readout_type = payload[4]
        ...
        try:
            decrypted = decrypt_victron_payload(payload, keys[mac])
    ...
    for mac in missing:
        print(f"{mac}  -- not seen in this scan")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Scan for and decode only the Victron Energy BLE Instant Readout "
            "devices listed in keys.json, using a BleuIO Pro USB BLE dongle."
        ),
    ...
    parser.add_argument("--scan-time", type=int, default=10, help="Seconds to run the discovery BLE scan (default: 10)")
    parser.add_argument("--target-scan-time", type=int, default=4, help="Seconds to target-scan each known device for its raw advertisement (default: 4)")
    parser.add_argument("--keys-file", default="keys.json", help="JSON file mapping MAC address -> Victron advertisement key (default: keys.json)")
    ...
    try:
        ...
        sys.exit(
            f"Keys file '{args.keys_file}' not found. This script only looks for "
            "devices listed there — see the --keys-file / keys.json format in "
            "the module docstring (python3 -m pydoc victron_bleuio_known_devices)."
        )
    if not keys:
        sys.exit(f"'{args.keys_file}' has no entries — nothing to scan for.")
